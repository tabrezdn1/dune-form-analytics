...
const CHANGE_MESSAGES = {
  ...
  FIELD_OPTIONS_CHANGED: (label: string) => 
    `Field "${label}" options changed`,
} as const
...
function checkFieldCompatibility(
  ...
  if (originalField.type !== currentField.type) {
    ...
    analysis.incompatibleChanges.push(
      CHANGE_MESSAGES.FIELD_TYPE_CHANGED(originalField.label, originalField.type, currentField.type)
    )
  ...
  if (originalField.label !== currentField.label) {
    ...
    analysis.compatibleChanges.push(
      CHANGE_MESSAGES.FIELD_LABEL_CHANGED(originalField.label, currentField.label)
    )
  }
  ...
  // Check for placeholder changes (compatible)
...
export function analyzeFormChanges(
  ...
  currentFields.forEach(currentField => {
    ...
    if (!originalField) {
      ...
      analysis.compatibleChanges.push(CHANGE_MESSAGES.FIELD_ADDED(currentField.label))
    } else {
  ...
  originalForm.fields?.forEach(originalField => {
    if (!currentFieldMap.has(originalField.id)) {
      ...
      analysis.incompatibleChanges.push(CHANGE_MESSAGES.FIELD_DELETED(originalField.label))
    }
  })
