...
class ApiClient {
  ...
  async getResponses(
    ...
    return this.request(
      `/api/forms/${formId}/responses?page=${page}&limit=${limit}`
    );
  }
  ...
  async getRealTimeMetrics(formId: string): Promise<
    ApiResponse<{
      activeUsers: number;
  ...
  async getTrendAnalytics(
    ...
    return this.request(
      `/api/forms/${formId}/trends?fieldId=${fieldId}&days=${days}`
    );
  }

  // WebSocket connection stats
  async getWebSocketStats(formId?: string): Promise<
    ApiResponse<{
