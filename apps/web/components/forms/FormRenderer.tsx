...
export function FormRenderer({
  ...
  const renderField = (field: FormField) => {
    ...
    switch (field.type) {
      case 'text':
        return (
          <TextInput key={field.id} {...commonProps} value={fieldValue || ''} />
        );
      ...
      case 'rating':
        return (
          <Rating key={field.id} {...commonProps} value={fieldValue || 0} />
        );
  ...
  if (visibleFields.length === 0) {
    return (
      <div className='text-center py-12'>
        <div className='text-gray-500 text-lg'>No fields to display</div>
      </div>
    );
  }
