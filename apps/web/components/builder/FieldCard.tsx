      ...
      case 'checkbox':
        return (
          <div className='mt-2 space-y-2'>
            {field.options?.slice(0, 3).map((option, index) => (
              <div key={option.id} className='flex items-center space-x-2'>
                <input type='checkbox' className='text-sm' disabled />
                <span className='text-sm text-gray-600 dark:text-gray-400'>
                  {option.label}
                </span>
              </div>
      ...
      case 'rating':
        return (
          <div className='mt-2 flex items-center space-x-1'>
            {Array.from(
              { length: Math.min(field.validation?.max || 5, 5) },
              (_, index) => (
                <svg
                  key={index}
    ...
    >
      ...
      <div className='flex items-center justify-between mb-2'>
        <div className='flex items-center space-x-2'>
          <div className={getFieldTypeColor()}>{getFieldTypeIcon()}</div>
          <div>
      ...
      <div className='mt-3 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400'>
        ...
        <div className='text-gray-400'>ID: {field.id}</div>
      </div>
    </div>
  );
}
