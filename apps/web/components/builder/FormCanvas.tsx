...
export function FormCanvas({
  ...
  const updateFieldPositions = useCallback(() => {
    ...
    const fieldElements =
      canvasRef.current.querySelectorAll('[data-field-index]');
  ...
  const handleCanvasOver = useCallback(
    (e: React.DragEvent) => {
      e.preventDefault();
      e.stopPropagation();
        ...
        <div className='text-center pointer-events-none'>
          ...
          <p className='text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto leading-relaxed'>
            <span className='hidden sm:inline'>
              Drag field types from the left palette to this area, or{' '}
            </span>
            <span className='sm:hidden'>Tap the buttons below or </span>
            <span className='hidden sm:inline'>
              use the quick-add buttons below
            </span>
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className='mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border'>
        ...
        <p className='text-sm text-gray-600 dark:text-gray-400'>
          <span className='hidden sm:inline'>
            Drag fields anywhere to reorder them or drop new fields from the
            palette.
          </span>
          <span className='sm:hidden'>
            Click fields to edit, tap and hold to reorder.
          </span>
        </p>
      </div>
