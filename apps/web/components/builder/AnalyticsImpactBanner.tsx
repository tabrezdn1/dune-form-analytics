...
export function AnalyticsImpactBanner({
  ...
  if (analysis.willResetAnalytics) {
    return (
      <div
        className={`rounded-lg border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 p-4 ${className}`}
      >
        <div className='flex'>
            ...
            <div className='mt-2 text-sm text-yellow-700 dark:text-yellow-300'>
              ...
              <ul className='mt-2 list-disc list-inside space-y-1'>
                {analysis.deletedFields.length > 0 && (
                  <li>
                    Deleted field{analysis.deletedFields.length > 1 ? 's' : ''}:{' '}
                    {analysis.deletedFields.join(', ')}
                  </li>
                )}
        ...
        <div className='flex'>
          ...
          <div className='ml-3'>
            ...
            <div className='mt-2 text-sm text-green-700 dark:text-green-300'>
              <p>
                Your {responseCount} response{responseCount !== 1 ? 's' : ''}{' '}
                and analytics data will be preserved.
              </p>
            </div>
          </div>
        </div>
...
export function AnalyticsImpactIndicator({
  ...
  return (
    <div
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 ${className}`}
    >
