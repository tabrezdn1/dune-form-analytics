...
import {
  ...
  AnalyticsImpactIndicator,
} from '@/components/builder/AnalyticsImpactBanner';
...
export default function FormBuilderClient({
  ...
  const [pendingSaveAction, setPendingSaveAction] = useState<
    'save' | 'publish' | null
  >(null);
  const [recentlyDeletedField, setRecentlyDeletedField] =
  ...
  const handleUndoDelete = (field: Field) => {
    ...
    if (initialForm?.fields) {
      const originalIndex = initialForm.fields.findIndex(
        f => f.id === field.id
      );
      if (originalIndex !== -1) {
  ...
  const handleSave = async (publish = false) => {
    ...
    if (
      initialForm &&
      changeAnalysis.willResetAnalytics &&
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
          <Breadcrumbs
            items={[
              ...
              {
                label: `${initialForm ? 'Edit' : 'Create'}: ${title || 'Untitled Form'}`,
                current: true,
              },
            ]}
          />
                ...
                {fields.length > 0 ? (
                  ...
                  <div className='bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-8 text-center'>
                    ...
                    <p className='text-lg text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto'>
                      Switch to the Build tab and add some fields to see the
                      preview
                    </p>
                    <button
                      onClick={() => setActiveTab('build')}
